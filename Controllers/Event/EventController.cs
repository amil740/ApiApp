using ApiProject.Data;
using ApiProject.DTOs.Event;
using ApiProject.DTOs.Organizer;
using ApiProject.Models;
using AutoMapper;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;

namespace ApiProject.Controllers
{
    [ApiController]
    [Route("api/[controller]")]
    public class EventController : ControllerBase
    {
        private readonly AppDbContext _context;
        private readonly IMapper _mapper;

        public EventController(AppDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }

        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var events = await _context.Events
                .ToListAsync();

            var eventDtos = _mapper.Map<List<EventDto>>(events);

            return Ok(eventDtos);
        }

        [HttpPost]
        public async Task<IActionResult> Create(EventCreateDto dto)
        {
            var organizerExists = await _context.Organizers
                .AnyAsync(o => o.Id == dto.OrganizerId);

            if (!organizerExists)
                return BadRequest("Organizer tapılmadı");

            var eventEntity = _mapper.Map<Event>(dto);

            await _context.Events.AddAsync(eventEntity);
            await _context.SaveChangesAsync();

            var eventDto = _mapper.Map<EventDto>(eventEntity);

            return Created($"/api/events/{eventEntity.Id}", eventDto);
        }

        [HttpPost("/api/events/{eventId}/banner")]
        public async Task<IActionResult> UploadBanner(int eventId, [FromBody] EventBannerUpdateDto dto)
        {
            var ev = await _context.Events.FindAsync(eventId);
            if (ev == null)
                return NotFound("Event tapılmadı");

            ev.BannerImageUrl = dto.BannerImageUrl;

            try
            {
                await _context.SaveChangesAsync();
            }
            catch (DbUpdateException ex)
            {
                return StatusCode(500, "Database error while updating banner: " + ex.Message);
            }

            return NoContent();
        }

        [HttpGet("/api/events/{eventId}/organizer")]
        public async Task<IActionResult> GetOrganizer(int eventId)
        {
            var ev = await _context.Events
                .Include(e => e.Organizer)
                .FirstOrDefaultAsync(e => e.Id == eventId);

            if (ev == null)
                return NotFound("Event tapılmadı");

            if (ev.Organizer == null)
                return NotFound("Organizer tapılmadı");

            var organizerDto = _mapper.Map<OrganizerDto>(ev.Organizer);

            return Ok(organizerDto);
        }
    }
}
